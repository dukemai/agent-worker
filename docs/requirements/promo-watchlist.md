...
| DELETE | `/api/promotions/current-week?storeKey=` | Cookie-auth. Deletes all current ISO-week promotion import runs, optionally scoped to one store; child `weekly_promotions` and `weekly_promotion_matches` cascade. Used by the Weekly offers tab to clear imported data before a clean reimport. |
...
2. **Scrape store offers** — from the repo root, run `pnpm promo:scrape:ica-maxi-barkarbystaden`, `pnpm promo:scrape:ica-nara-kallhall`, `pnpm promo:scrape:coop-kallhall`, or `pnpm promo:scrape:coop-barkarby` (network; writes **`<store-key>-scraped-promotions.json`** with all offers for that store). From `apps/playwright-tools`, the equivalent scripts omit the `promo:` prefix.
