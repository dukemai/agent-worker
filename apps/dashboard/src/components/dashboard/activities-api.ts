...
export type ManualActivityPayload = {
  title: string; description: string | null; area: string | null; address: string | null;
  activity_type?: string; location_url?: string | null; booking_url?: string | null;
  valid_from?: string | null; valid_until?: string | null; season?: string; time_text?: string | null;
  cost_level: string; price_text: string | null; cost_notes: string | null; booking_required: boolean;
  booking_notes?: string | null; booking_deadline?: string | null; weather_fit: string; energy_level: string;
  age_min: number | null; age_max: number | null; age_notes: string | null; usual_duration_minutes?: number | null; tags: string[];
};

export async function createManualLocalActivity(payload: ManualActivityPayload): Promise<{ activity: LocalActivity }> {
  const response = await fetch("/api/activities/local", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
  if (!response.ok) await readApiError(response, "Failed to create activity");
  return response.json();
}

export async function createManualSeasonalActivity(payload: ManualActivityPayload): Promise<{ instance: SeasonalActivityInstance }> {
  const response = await fetch("/api/activities/seasonal", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
  if (!response.ok) await readApiError(response, "Failed to create seasonal activity");
  return response.json();
}
